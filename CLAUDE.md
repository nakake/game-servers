...
`registry.json` を変更したら `scripts/register-game.sh <game_id>` で Workers KV に反映する(Phase 2 以降に実装)。
...
- `terraform.tfstate` は **S3 + DynamoDB lock** で remote backend (Phase 4 で設定)。
...
4. `scripts/register-game.sh <new-game>` で Cloudflare DNS + Workers KV に反映
5. Discord で `/list` に出現することを確認
