// GAME_REGISTRY の game_id 一覧 (registry-index) のキー名・TTL・検証。
//
// index は SERVER_STATE KV に置くキャッシュで、値は game_id の JSON 配列 (昇順)。
// source of truth は GAME_REGISTRY のキーそのもの — index が無い / 壊れているときは
// list から作り直す (lib/registry/store.ts)。
// scripts/register-game.mjs は TS を import できないため、キー名と TTL を複製して持つ。
// 変えるときは両方直すこと。
//
// phase7-webui ブランチでは同じ内容を packages/shared/src/registry-index.ts に置いている
// (admin-webui と共有するため)。master には @gs/shared がまだ無いのでここに置く。
