...
最終更新: 2026-05-24 (rev3: rev2 への security 着手 OK 判定での条件付き指摘 5 件を反映、tag backfill 前提条件 / 緊急 rotation 代替手順 / Step 7 dead window / staging tfstate 分離 等)
...
- [ ] **Cloudflare WAF rate limit** を `/oidc/*` に設定 (IP 単位 60 req/min) — dashboard で手動、手順は runbook に。**注: rate limit は best-effort** (IP rotate / Tor / proxy で容易にバイパス可)。JWKS endpoint の **主防御は edge cache (`s-maxage=86400`)** によるオリジン到達抑制で、WAF は二次防御
...
- [ ] **`docs/runbook-phase5-oidc.md` 新規**:
  ...
  - **Cloudflare WAF rate limit 設定** (Step 1.5 で設定したルールの確認手順、将来的に `infra/cloudflare/` で Terraform 化を検討)
- [ ] **`docs/design.md` 更新**:
...
| JWKS endpoint への DoS / 無料枠消費 | **主防御**: `Cache-Control s-maxage=86400` で edge cache (オリジン到達抑制)。**二次防御**: Cloudflare WAF rate limit 60 req/min/IP (best-effort、IP rotate でバイパス可) |
...
- [ ] **Cloudflare WAF を Terraform IaC 化**: 現状ダッシュボード手動設定。`infra/cloudflare/` 配下に WAF rule を Terraform で入れる検討。Phase 5 完了後に判断 (Step 8 メモ)
