...
最終更新: 2026-05-31 (rev4: admin WebUI を別 Worker `admin-webui` に分離 + AWS は Service Binding 委譲 = ADR 0004。rev3: opaque token / 2 ティア権限 / logout 案 b。rev2: 独立検証 Phase A 廃止)
...
```
[Discord] (gs-discord-handler 上の /admin command)
   │  /admin
   ▼
...
[admin-webui Worker /admin/api/*]
   ...
   │  ④b start / stop / status / S3 config sync     ← Service Binding で委譲 ↓
   ▼
[discord-handler Worker /internal/*] (service-binding 専用)
...
- **AWS 鍵の隔離**: admin-webui は OIDC 秘密鍵を持たない。AWS に触る操作 (start/stop/status/S3) のみ **Service Binding** で discord-handler の `/internal/*` に委譲。KV/CF/DNS は admin-webui が自前 (ADR 0004)
- **Source of truth は Workers KV**: WebUI 操作は KV を直接書く。Git の `games/<id>/registry.json` は audit 用に monthly export スクリプトで反映 (詳細 §8)
- **Discord OAuth 不使用**: magic link 認証で Discord 既存 bot の認証に乗っかる (ADR 0003)。`/admin` command は引き続き discord-handler 上 (token 発行のみ)、それ以外の WebUI は admin-webui
...
Discord 側にも `/admin logout` を用意し、自分の全 session を失効する経路を持たせる (デバイス紛失対策)。
...
| `ADMIN_AUTH` (新) | `admin_token:*` / `admin_session:*` | **新規** | 両 Worker (`/admin` token 発行は discord-handler、検証は admin-webui) |

`SERVER_STATE` に相乗りも可能だが、key prefix 衝突と TTL の独立性のため `ADMIN_AUTH` を別 namespace で作る。**同じ namespace id を両 Worker の `wrangler.toml` に bind する** (KV namespace は複数 Worker から共有可能)。`admin_token:*` は discord-handler の `/admin` command が put し admin-webui の `/auth` が読む、`admin_session:*` は admin-webui のみが扱う。
...
| `POST` | `/admin/api/games/:id/start` | 起動 | player | **Service Binding** → discord-handler `/internal/start` (AWS) |
| `POST` | `/admin/api/games/:id/stop` | 停止 | player | **Service Binding** → discord-handler `/internal/stop` (AWS) |
| `GET` | `/admin/api/games/:id/status` | 状態取得 | player | **Service Binding** → discord-handler `/internal/status` (EC2 describe) + KV state read |
...
AWS に触る操作 (start/stop/status/S3 config sync) は **admin-webui には置かず、discord-handler の内部 endpoint に Service Binding 経由で委譲**する (ADR 0004、鍵の隔離)。orchestrator core は **discord-handler 内に閉じたまま**、Discord interaction と内部 endpoint の両方から呼べるように切り出す:

```
discord-handler Worker
  handlers/discord/start.ts    ── Discord 用 wrapper (3秒応答 + followUp)
  handlers/internal/start.ts   ── service-binding 用 wrapper (JSON 応答)
        ↓                              ↓
        └─── lib/orchestrator/start.ts (game, ctx, env, progressCallback) ───┘

admin-webui Worker
  handlers/admin/games.ts      ── env.DISCORD_HANDLER.fetch('/internal/start', ...) を叩くだけ
```

- **内部 endpoint (`/internal/*`) は service-binding 経由のみ許可**: 外部からの直叩きを弾く (binding 経由か header で判定)。public route には生やさない
- **進捗通知**: Discord 側は `followUp.editOriginal`、Web 側は **polling で `/admin/api/games/:id/status`** (これも内部 status を Service Binding で引く)。SSE は MVP では使わない
- orchestrator を「共有 npm package」にせず discord-handler 内に置くのは、AWS 認証情報 (OIDC 鍵) を admin-webui に漏らさないため。admin-webui が import するのは型定義程度に留める
...
└─ src/
    ...
    │   ├─ games.ts              # /admin/api/games/* (AWS 操作は DISCORD_HANDLER binding に委譲)
    ...
    └─ lib/
        ...
        └─ registry/
            └─ build.ts          # 新規 (NewGameForm → GameDefinition 変換、(form, tier) を受ける)

workers/discord-handler/         # 既存。Phase 7 で追加するのは下記のみ
└─ src/
    ├─ handlers/
    │   ├─ discord/
    │   │   └─ admin.ts          # 新スラッシュコマンド /admin (token 発行 + ephemeral button)
    │   └─ internal/             # 新規。service-binding 専用 (AWS 操作)
    │       ├─ start.ts
    │       ├─ stop.ts
    │       ├─ status.ts
    │       └─ s3-sync.ts
...
discord-handler 側 `wrangler.toml` には `ADMIN_AUTH` の bind 追加 (`/admin` token 発行用) と、`/internal/*` route の実装が増えるのみ。`[assets]` は **付けない** (SPA を持たない)。
...
| # | リスク | 対策 |
|---|---|---|
| 1 | `/admin` 応答に EPHEMERAL flag を忘れる (**load-bearing**: token は bearer で allowlist は漏洩を止めない → チャンネル全員が 5 分以内にクリックすれば発行者として入れる) | unit test で `flags: 64` を assert (最重要) |
...
| 8 | session 24h が長すぎてデバイス紛失時に被害 | `/admin logout` (Discord 経由) で全 session revoke 可能に (sid 列挙の仕組みは §4.4 で要設計) |
...
| Step | 内容 | 推定 | 依存 |
|---|---|---|---|
| **B-0** | `workers/admin-webui/` scaffold (wrangler.toml: assets + KV bind + DISCORD_HANDLER service binding)、独自ドメイン `gs-admin.<base-domain>` 割当、空 deploy 疎通 | 2h | なし |
| **B-1** | CurseForge API client (`admin-webui/src/lib/curseforge/`) 実装 + 単体テスト | 2h | B-0 |
| **B-2** | Magic link auth: `admin-webui` の `lib/auth/admin-session.ts` + `handlers/auth.ts` (session/tier) と、discord-handler の `handlers/discord/admin.ts` (token 発行 + ephemeral button、ADMIN_AUTH bind) | 4h | B-0 |
...
| **D-2** | `/admin/api/games` POST (新規追加) + DNS (admin-webui 直) / KV / **S3 sync は Service Binding 経由** (`register-game.mjs` の主要処理を `lib/registry/build.ts` に移植、`(form, tier)` 対応) | 5h | D-1 |
...
| **E-1** | discord-handler: `lib/orchestrator/start.ts` `stop.ts` 切り出し + `handlers/internal/{start,stop,status,s3-sync}.ts` (service-binding 専用 endpoint) + Discord handler の refactor | 4h | D-3 |
| **E-2** | admin-webui `/admin/api/games/:id/{start,stop,status}` (DISCORD_HANDLER binding を叩く) + SPA: ops 画面 | 4h | E-1 |
| **F** | `scripts/export-registry.mjs` (KV → Git audit) + 月次運用手順を runbook に追記 | 2h | E-2 |

**Total**: ~41h。MVP (B-0〜C-3 まで = 既存 game の version 編集が WebUI でできる状態) なら ~20h。

> 分離 (ADR 0004) による増分: B-0 (Worker scaffold +2h) と E-1 の内部 endpoint 化 (+1h)。代わりに `run_worker_first` ディスパッチの実装・テスト・本番事故リスクが消える。
