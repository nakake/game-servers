...
Spot 中断を擬似発火する手段は AWS には 2 種類ある:

| 方式 | コスト | テスト範囲 | おすすめ |
|---|---|---|---|
| **B1: SNS topic に直接 publish** | 0 円、すぐ動く | Worker `/aws/notification` 以降の全部 (整形 / Discord 配信) | ✅ 通常の検証 |
| **B2: AWS FIS で本物の Spot 中断** | 起動中 EC2 が **実際に terminate される** | EventBridge rule + input_transformer も含めた完全 e2e | EventBridge 設定を疑うときだけ |

> **重要**: AWS CLI に `aws ec2 send-spot-instance-interruption` という command は **存在しない**
> (古いドキュメントや AI 生成手順で出てくる誤情報)。FIS の action 名 `aws:ec2:send-spot-instance-interruptions`
> と混同しないこと。

### B1: SNS publish で擬似発火 (推奨)

EventBridge の `input_template` が SNS に出す payload を直接再現する。Worker の整形分岐
(`isSpotInterruptionMessage` の prefix detection) と Discord 配信を実機で確認できる。

```powershell
# SNS topic ARN を取得 (既知なら直書き OK)
$topicArn = aws sns list-topics `
  --region ap-northeast-1 `
  --query 'Topics[?contains(TopicArn, `:gs-alerts`)].TopicArn | [0]' `
  --output text

# EventBridge input_template (infra/envs/prod/eventbridge.tf) と同じ文面で publish。
# instance-id / region / action / time は適当な値で OK (整形ロジックの検証なので)。
$now = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
$message = "Spot interruption warning: instance i-fake0123456789 (ap-northeast-1); action=terminate; reclaimed in ~2 min; event time $now"
...
数秒以内に Discord channel に下記 embed が出れば成功。

### B2: AWS FIS で本物の Spot 中断 (任意)

EventBridge rule / input_transformer まで含めた完全な e2e を試したいときのみ。**ATM11 を
起動中に発火させると本当に 2 分後に terminate される** ので、起動直後 (world データがまだ
進んでいないタイミング) で発火させること。

簡単なのは AWS-公式のラッパ CLI `aws/amazon-ec2-spot-interrupter` を使う方式:

```powershell
# Go が入っていれば 1 行
go install github.com/aws/amazon-ec2-spot-interrupter@latest

# 起動中の atm11 instance ID を取って発火
$instanceId = aws ec2 describe-instances `
  --filters "Name=tag:Game,Values=atm11" "Name=instance-state-name,Values=running" `
  --query 'Reservations[0].Instances[0].InstanceId' `
  --output text `
  --region ap-northeast-1
ec2-spot-interrupter --instance-ids $instanceId --region ap-northeast-1
```

Go を入れたくない場合は AWS FIS console から experiment template を作って手動実行する手も
あるが (action: `aws:ec2:send-spot-instance-interruptions`、IAM role 必要)、初回セットアップ
が重いので **B1 で通常の検証は済ませる**のがおすすめ。
...
| 症状 | 確認 |
|---|---|
| B1 (SNS publish) で Discord に何も来ない | (1) `aws sns list-subscriptions-by-topic --topic-arn $topicArn` で Worker endpoint subscription が `Confirmed` か (2) `wrangler tail` で `/aws/notification` が叩かれているか (3) `SNS_ALLOWED_TOPIC_ARN` env が本番 topic と一致しているか |
| B2 (FIS) では届くが B1 で届かない | publish した message の prefix が `Spot interruption warning:` (大文字 S 始まり) になっているか確認 |
| B2 (FIS) で届かない | (1) `aws events list-rules --name-prefix gs-spot` で rule 存在を確認 (2) EventBridge → SNS の権限 (events.amazonaws.com → SNS:Publish) が `infra/envs/prod/sns.tf` の topic policy で許可されているか |
| `🚨 AWS notification` という generic title で出る | `isSpotInterruptionMessage` の prefix 検出が外れた。message が `Spot interruption warning:` で始まっているか確認 |
