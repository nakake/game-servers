...
- **Status**: Accepted
- **Date**: 2026-05-27 (Accepted 2026-05-31)
- **Deciders**: nakake
- **Related**: [phase7-modpack-webui.md](../phase7-modpack-webui.md) §4 / [adr/0004-admin-webui-separate-worker.md](0004-admin-webui-separate-worker.md) / [design.md](../design.md) §9 (セキュリティ)
- **改訂**: 2026-05-31 の design review で (a) session を JWT → **opaque KV token** (署名鍵を増やさない)、(b) **2 ティア権限** (player/admin) を導入、(c) Discord コマンドを `/admin` → **`/panel`** に改名、(d) token = bearer / EPHEMERAL が load-bearing / TTL 600s→300s を明記。詳細は phase7 doc §1.1 / §4。
...
**Discord ephemeral message 経由の magic link + opaque token cookie session** を採用する。

> **session token の方式 (rev: opaque token, JWT 不採用)**: cookie には署名済 JWT ではなく **乱数の opaque session id** を入れ、実体は KV `admin_session:<sid>` に持つ (stateful)。理由: middleware で毎リクエスト KV を引く設計なので、JWT の署名検証はセキュリティ上ほぼ無価値 (偽造 cookie は KV miss で弾ける / 生きた sid を当てるのは乱数強度で不可能)。一方 JWT を選ぶと **HS256 署名鍵という新規 Workers Secret が 1 個増え**、本 ADR の要件 #2 (secret 管理の最小化) と矛盾する。opaque token なら新規 secret ゼロ・実装最小 (JWT lib 不要)・revocation も KV delete 1 行で、JWT と同等のセキュリティを得られる。完全 stateless JWT (KV を引かない) は即時 revocation を失うため、AWS 制御権を握る admin panel には不適。

### フロー

1. Discord で `/panel` スラッシュコマンドを実行
2. Worker が:
   - 32 bytes CSPRNG token を生成
   - KV `admin_token:<token>` = `{user_id, exp_ts, used:false}` を **TTL 300s** で put (URL に乗る token の漏洩 exposure 窓を縮めるため短め。ユーザーは発行直後にクリックするので 5 分で十分)
...
4. Worker が:
   ...
   - `user_id` が `ADMIN_DISCORD_USER_IDS` か `PLAYER_DISCORD_USER_IDS` (CSV env) に含まれるか確認し tier を決定 (どちらにも無ければ reject / fail-closed)。tier ごとの操作権限 = 認可は phase7 doc §1.1 に分離 (本 ADR は認証方式の決定に集中)
   - 32 bytes CSPRNG の opaque session id (`<sid>`) を生成し `gs_admin_session` cookie で発行 (`Secure; HttpOnly; SameSite=Strict; Path=/`)
   - KV `admin_session:<sid>` = `{user_id, issued_at, exp_ts}` を TTL 86400s で put (この KV エントリが session の実体 = revoke 可能)
   - HTML 応答内で `<script>history.replaceState(null,'','/');location.href='/'</script>` を実行し URL から `?t=` を消す
5. 以降の `/admin/api/*` アクセスは cookie の `<sid>` → KV `admin_session:<sid>` 存在チェック + allowlist 再確認の middleware を通す
6. 状態変更 API (PUT/POST/DELETE) は `X-Requested-With: fetch` ヘッダ要求で CORS preflight を強制 (CSRF 軽減)
7. logout は cookie clear + KV `admin_session:<sid>` delete。Discord 側 `/panel logout` で自分の全 session を失効する経路も持たせる (全 session 列挙の仕組みは別途検討、phase7 doc §4.4 / §5.2)

### コードの責務分割

- `lib/auth/admin-session.ts` — magic-link token issue / verify / opaque session token issue・verify / allowlist check (署名鍵は使わない、すべて KV stateful)
- `handlers/discord/panel.ts` — `/panel` slash command (token 生成 + ephemeral button 応答)
...
1. **Discord OAuth app の登録・client_secret 管理が不要**。session を opaque token + KV にしたため **認証まわりで新規 Workers Secret は増えない** (CF API key 以外、Phase 7 で増える secret はゼロ)
...
7. **session revoke が KV で 1 行**: デバイス紛失時に Discord 側 `/panel logout` で全 session 失効可能 (sid 列挙の仕組みは要設計、phase7 doc §4.4)

### Negative / Trade-off

| リスク | 緩和策 |
|---|---|
| **Token が URL に乗る** (履歴 / proxy log / referrer に漏れる可能性) | (a) `/auth` 応答で `history.replaceState` を即実行し URL から消す, (b) Discord 側で **link button UI** を採用し、URL を平文テキストでチャット履歴に残さない, (c) **TTL を 300s に短縮**し漏洩 exposure 窓を縮める, (d) one-shot (`used=true`) |
| **token は bearer (持つ者が発行者として通る)** — `/auth` は token に紐づく *発行者* の user_id を allowlist 照合するのであって *クリックした人* を再認証しない (ブラウザに Discord ID がないため。OAuth を避けた代償)。**∴ allowlist は token 漏洩を backstop しない** | 漏洩対策は「漏らさない (ephemeral) / 一度しか使えない (one-shot) / 短時間で失効 (TTL 300s)」の 3 点に集約される。これは email magic link と同じ性質で新規リスクではない |
| **EPHEMERAL flag が load-bearing** — `flags: 64` を忘れると token URL がチャンネル全員に見え、上記の通り allowlist では止まらない (チャンネル内の誰でも 5 分以内にクリックすれば発行者として入れる) | unit test で `flags: 64` を必ず assert (本 ADR で最も重要なテスト) |
...
| **24h session が残ったままデバイス紛失** | Discord 側 `/panel logout` で全 session 失効 (列挙方式は phase7 doc §4.4 で別途設計) |
