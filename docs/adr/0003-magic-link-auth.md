...
7. logout は cookie clear + KV `admin_session:<sid>` delete。Discord 側 `/admin logout` で自分の全 session を失効する経路も持たせる (全 session 列挙の仕組みは別途検討、phase7 doc §4.4 / §5.2)
...
7. **session revoke が KV で 1 行**: デバイス紛失時に Discord 側 `/admin logout` で全 session 失効可能 (sid 列挙の仕組みは要設計、phase7 doc §4.4)
...
| **24h session が残ったままデバイス紛失** | Discord 側 `/admin logout` で全 session 失効 (列挙方式は phase7 doc §4.4 で別途設計) |
