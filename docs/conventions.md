...
このリポジトリで作業する人 / agent 向けの規約集。設計の意図と運用上の制約をまとめる。詳細な背景は `docs/design.md` を参照。
...
- AMI は **汎用一個** で全ゲーム対応。ゲーム別ロジックは sidecar 内の adapter (`launcher/sidecar/src/adapters/`) と registry に閉じる。
- AMI 再ビルドのタイミング: Docker / Java / launcher / **sidecar** (Phase 3 で同梱) の変更時のみ。
- ゲーム設定変更は S3 sync で済むため AMI 再ビルド不要。
- **sidecar image は AMI に焼き込み** (`/var/lib/sidecar-image.tar`、`docker load` は cloud-init 側で実行)。AMI ビルドは `scripts/build-sidecar-ami.ps1` 一発 (npm build → docker build → docker save → packer build)。新 AMI ID は `aws ssm put-parameter --overwrite --name /gs/ami/game-server-latest` で SSM に書き込めば Launch Template が次回起動から自動参照する (terraform apply 不要)。詳細 `docs/runbook-phase3-sidecar.md` Step 5。
