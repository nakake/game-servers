...
- [x] **ユーザー実行** (完了 2026-05-23): `docs/runbook-phase3-sidecar.md` Step 1 (SSM 投入) と Step 2 (Wrangler secret 投入) を実機で実行。`aws ssm get-parameter --name /gs/atm11/sidecar_hmac_secret` と `pnpm wrangler secret list` で両側の存在を確認済
...
- [x] **ユーザー実行** (完了 2026-05-23): ① `terraform apply` で SSM Parameter 作成 + LT image_id 切替 (初回 apply で `data_type = "aws:ec2:image"` 指定漏れによる EC2 SsmInvalidParameter を踏み、`ami.tf` を修正 → 再 apply で解消)、② `scripts/build-sidecar-ami.ps1` で AMI build (途中で `install-sidecar.sh` の `tar -tf | head -5` が pipefail で誤検知する bash バグを踏み、validation/preview を分離して解消)、③ `aws ssm put-parameter --overwrite` で SSM `/gs/ami/game-server-latest` に新 AMI ID を反映、④ smoke check はスキップ (Step 8 実機確認で代替)
...
- [x] **AMI ID の参照経路** (確定 2026-05-23 — Step 7): SSM Parameter Store `/gs/ami/game-server-latest` を採用。Launch Template の `image_id` は `resolve:ssm:/gs/ami/game-server-latest` 形式で **起動時解決** (data source 参照ではない、terraform apply 不要で AMI 更新が反映できる)。SSM パラメータには `data_type = "aws:ec2:image"` を必ず指定 (EC2 が image data type を要求するため、未指定だと CreateLaunchTemplateVersion が SsmInvalidParameter で落ちる)。Packer build orchestrator (`scripts/build-sidecar-ami.ps1`) の出力した AMI ID を `aws ssm put-parameter --overwrite` で反映するワークフロー
