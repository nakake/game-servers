...
1. **import 中心、再作成は最後の手段**。手動で動いているリソースは原則 `terraform import` で TF 管理下に取り込む (再作成すると依存リソースの ARN/ID が変わり Worker / wrangler.toml / 既存 EBS snapshot タグなどに波及する)。例外: SNS topic は `CreateTopic` 冪等なので「import せず apply」で同等の効果。
...
| IAM | Role inline policy (SSM/S3 系既存) | (既存複数) | `import` |
| IAM | Role inline policy (SNS publish) | `gs-phase0-ec2-sns-publish` | **既に着手済** (Step 0) |
| IAM | User + Access Key | `gs-worker-caller` | `import` (Access Key 値は sensitive output) |
| IAM | User policy | `gs-worker-caller-policy` | `import` |
| Storage | S3 bucket | `gs-game-configs` | `import` (versioning, lifecycle 同時) |
| Storage | SSM Param (SecureString) | `/gs/atm11/rcon_password` | `import` または除外判断 |
| Notification | SNS topic | `gs-alerts` | **既に着手済** (Step 0、CreateTopic 冪等で自動取り込み) |
...
### Step 0: SNS / Budget / EC2 inline policy を apply  *(HCL 着手済 / apply 未)*

- [ ] `winget install Hashicorp.Terraform`
- [ ] `infra/envs/prod/terraform.tfvars` 作成、`worker_notification_url` を埋める
- [ ] `terraform init` / `terraform plan -out=tfplan`
- [ ] `terraform apply tfplan`
- [ ] 旧手動 HTTPS subscription を SNS Console で削除
- [ ] 旧手動 Budget を Budgets Console で削除
- [ ] `aws sns publish` smoke test → Discord 到達確認
- [ ] `/start atm11` → user-data ログで `aws sns publish` が `AuthorizationError` を出さないこと確認 → Discord に「✅ 接続可能になりました」が届くこと
...
- [ ] `aws_iam_role.gs_phase0_ec2` を import
- [ ] `aws_iam_instance_profile.gs_phase0_ec2` を import
- [ ] role に attach 済の既存 inline policy (SSM GetParameter, S3 GetObject 系) を `aws_iam_role_policy` として import
- [ ] Step 0 で書いた `aws_iam_role_policy.ec2_sns_publish` の `role` 属性を文字列リテラルから `aws_iam_role.gs_phase0_ec2.name` に書き換え
- [ ] `aws_iam_user.gs_worker_caller` を import
- [ ] `aws_iam_user_policy.gs_worker_caller` (または attached managed) を import
- [ ] **Access Key** は import せず、新しい Access Key を Terraform で発行 → Worker secret 更新 → 旧 Key を IAM Console で無効化、の rotation サイクルにする (Phase 2 で OIDC 化したら全廃)

Worker コード変更: なし。  
Worker secrets 変更: Access Key rotation を伴う場合のみ `wrangler secret put AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`。
...
### Step 3: Storage の import

- [ ] `aws_s3_bucket.gs_game_configs` を import
- [ ] `aws_s3_bucket_versioning` / `aws_s3_bucket_public_access_block` / `aws_s3_bucket_server_side_encryption_configuration` を新規宣言 (現状の状態に合わせる)
- [ ] `aws_s3_bucket_lifecycle_configuration` を新規 (modpacks/ は永続、launcher/ は新版上書きのみ)
- [ ] SSM Parameter `/gs/atm11/rcon_password` の扱いを決める:
  - 案 a. `aws_ssm_parameter` を import (値が state に入る)
  - 案 b. IaC 外に置き続け、Worker / EC2 は path だけ参照
  - **推奨**: b。秘密値を state に出さない方針。生成手順は runbook に残す
...
| リソース | state に入る値 | 対処 |
|---|---|---|
| `aws_iam_access_key` | secret access key | import せず TF で再発行 (Step 1) |
