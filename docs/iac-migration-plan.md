...
| IAM | ~~User `gs-worker-caller`~~ | ~~`gs-worker-caller`~~ | ~~`import`~~ → Phase 5 (2026-05-24) で削除済。Worker 認証は OIDC AssumeRoleWithWebIdentity (`gs-worker-oidc-role`) に移行 |
| IAM | ~~User policy~~ | ~~`gs-worker-caller-policy`~~ | ~~`import`~~ → Phase 5 で削除済。新 `gs-worker-oidc-policy` (least privilege、tag/LT/InstanceType 制約) を OIDC role に attach |
| IAM | OIDC provider + Role | `gs-worker-oidc-role` | Phase 5 で新規追加 (Terraform module `infra/modules/aws-oidc-cloudflare/`) |
...
- [x] **Access Key** は IaC 管理外で確定 (ユーザー判断)。既存キー `AKIA...JCUVJ74` をそのまま運用し、~~Phase 2~~ Phase 5 (2026-05-24) の OIDC 移行でキー方式ごと廃止済 (`aws iam delete-access-key` + IAM user `gs-worker-caller` 自体を `terraform apply` で削除、rotation は不要)
...
- [x] **Step 5 後の Worker → AWS 認証**: Phase 5 (2026-05-24) で OIDC AssumeRoleWithWebIdentity に移行完了。詳細は `docs/phase5-plan.md` + `docs/runbook-phase5-oidc.md`。Access Key rotation は実施せず、Phase 5 で keys ごと廃止
