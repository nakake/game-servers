...
## Step 3: Worker をデプロイ (Step 8 のプリチェック)

`SIDECAR_HMAC_SECRETS` 必須化 (Phase 3 Step 1 で `env.ts` を変更済) のため、Step 5 完了後で
ないと Phase 3 デプロイは Worker 起動エラーで弾かれる。デプロイ自体は Step 8 で行うが、
ここで `pnpm build` (dry-run) が通ることだけ確認する:

```powershell
cd F:\project\game_servers\workers\discord-handler
pnpm build
# → "Your worker has access to the following bindings:" に SIDECAR_HMAC_SECRETS が出れば OK
```

## Step 4: 動作確認 (Step 8 で再テストするので軽め)
...
- [ ] `pnpm build` (Worker dry-run) が `SIDECAR_HMAC_SECRETS` を含む bindings 一覧を表示
