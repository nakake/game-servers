...
- [x] **③ KV 投入**: `wrangler kv key put --binding GAME_REGISTRY <game_id> --path <registry.json>`。
      `--namespace-id` ではなく `--binding` を使い、namespace ID は `wrangler.toml` から解決させる
      (ID の二重管理を避ける)。binding 未追加 (Step 3 前) なら分かりやすく停止する
      (※当初計画の表記には `--remote` が入っていたが、wrangler の kv key put/get は本番 KV が
      default で `--remote` フラグは存在しない。Step 3 でバグを修正した経緯あり、本表記は修正後)
